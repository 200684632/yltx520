...
class FeelingAction extends CommonAction {
	public function index() 
	{
		...
		$whereq = array('flag'=>array('NEQ',-1),'pid'=>$pid);
	...
	public function update(){
		if(isset($_GET['id'])){
			...
			$goods =M('article')->where($where)->select();
			$this->goods = $goods;
			$this->display();
		}
	}
		...
		if (!empty($_POST['name'])){
			if ($_POST['intro']){
				if ($path){
					$data = array(
						'title'=>$_POST['name'],
						'content'=>str_replace('"\"', '', $_POST['intro']),
				...
				$query  = M('article')->where($where)->save($data);
				if($query){
					$this->success('添加成功');
				}else {
					$this->error('修改失败');
	...
	public function add() {
		...
		//            $this->error('文件上传失败');
			...
			if ($_POST['pid']){
				if (!empty($_POST['name'])){
					if (count($_POST['desc'])){
						$data = array(
							'title'=>$_POST['name'],
							'content'=>stripslashes($_POST['intro']),
							'pid'=>$_POST['pid'],
							'creattime'=>$_POST['time'],
							'describe'=>$_POST['desc'],
							'image'=>$path
			...
			$one = M('class')->where(array('pid'=>0,'show'=>0))->select();
			foreach ($one as $k=>$v)
			{
				$two = M('class')->where(array('pid'=>$v['id']))->select();
				$one[$k]['two'] = $two;
			}
			$this->one = $one;
			$this->display();
...
//详细图片上传
	...
	{
		$one = M('class')->where(array('pid'=>0,'show'=>0,'flag'=>array('NEQ',-1)))->select();
		foreach ($one as $k=>$v)
		{
			$two = M('class')->where(array('pid'=>$v['id']))->select();
			$one[$k]['two'] = $two;
